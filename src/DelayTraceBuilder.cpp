/* Copyright (C) 2016 Stavros Aronis
 *
...
DelayTraceBuilder::DelayTraceBuilder(const Configuration &conf) : TSOPSOTraceBuilder(conf) {
...
DelayTraceBuilder::~DelayTraceBuilder(){
}

bool DelayTraceBuilder::schedule(int *proc, int *aux, int *alt, bool *dryrun){
  *dryrun = false;
  *alt = 0;
  this->dryrun = false;
  if(replay){
   ...
   *
   * Round robin schedule, including real and aux.
   */
  const unsigned sz = threads.size();
  unsigned p, op, wrap;
  if(prefix_idx > 0){
    op = p = prefix[prefix_idx-1].iid.get_pid();
  }else{
    op = p = 0;
  }
  for(wrap = 0; p != op || wrap != 1;){
    if(threads[p].available && !threads[p].sleeping &&
       (conf.max_search_depth < 0 || threads[p].clock[p] < conf.max_search_depth)){
      ++threads[p].clock[p];
      // llvm::dbgs() << " Available: " << p << "\n";
      Event evt(IID<IPid>(IPid(p),threads[p].clock[p]),threads[p].clock,bnd);
      unsigned q = (p + 1) % sz;
      if(p + 1 >= sz){
        wrap = 1;
      }else{
      ...
      op = p;
      for(; bnd > 0 && (q != op || wrap != 1);){
        if(threads[q].available && !threads[q].sleeping &&
           (conf.max_search_depth < 0 || threads[q].clock[q] < conf.max_search_depth)){
          Branch cand = {q,0};
          // llvm::dbgs() << "   Also: " << q << "\n";
          evt.branch.insert(cand);
          bnd--;
        }
        if(q + 1 >= sz){
          wrap = 1;
...
void DelayTraceBuilder::refuse_schedule(){
...
void DelayTraceBuilder::mark_available(int proc, int aux){
  threads[ipid(proc,aux)].available = true;
}

void DelayTraceBuilder::mark_unavailable(int proc, int aux){
  threads[ipid(proc,aux)].available = false;
}

void DelayTraceBuilder::metadata(const llvm::MDNode *md){
...
bool DelayTraceBuilder::sleepset_is_empty() const{
...
bool DelayTraceBuilder::check_for_cycles(){
  IID<IPid> i_iid;
...
Trace *DelayTraceBuilder::get_trace() const{
...
bool DelayTraceBuilder::reset(){
  if(conf.debug_print_on_reset){
    llvm::dbgs() << " === DelayTraceBuilder reset ===\n";
    debug_print();
    llvm::dbgs() << " =============================\n";
  }
  ...
  /* Setup the new Event at prefix[i] */
  {
    ...
    if (conf.dpor_mode == Configuration::DELAY_SLEEP){
...
IID<CPid> DelayTraceBuilder::get_iid() const{
  IPid pid = curnode().iid.get_pid();
...
std::string DelayTraceBuilder::iid_string(const Event &evt) const{
  std::stringstream ss;
...
void DelayTraceBuilder::debug_print() const {
  llvm::dbgs() << "DelayTraceBuilder (debug print):\n";
  int iid_offs = 0;
  int clock_offs = 0;
...
void DelayTraceBuilder::store(const ConstMRef &ml){
  if(dryrun) return;
...
void DelayTraceBuilder::atomic_store(const ConstMRef &ml){
  if(dryrun){
...
void DelayTraceBuilder::load(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
void DelayTraceBuilder::full_memory_conflict(){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
void DelayTraceBuilder::join(int tgt_proc){
  if(dryrun) return;
  IPid ipid = curnode().iid.get_pid();
...
void DelayTraceBuilder::mutex_lock(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
void DelayTraceBuilder::mutex_lock_fail(const ConstMRef &ml){
  assert(!dryrun);
  if(!conf.mutex_require_init && !mutexes.count(ml.ref)){
    // Assume static initialization
    mutexes[ml.ref] = Mutex();
...
void DelayTraceBuilder::mutex_trylock(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
void DelayTraceBuilder::mutex_unlock(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
void DelayTraceBuilder::mutex_init(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
void DelayTraceBuilder::mutex_destroy(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
bool DelayTraceBuilder::cond_init(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
bool DelayTraceBuilder::cond_signal(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
bool DelayTraceBuilder::cond_broadcast(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
bool DelayTraceBuilder::cond_wait(const ConstMRef &cond_ml, const ConstMRef &mutex_ml){
  {
...
int DelayTraceBuilder::cond_destroy(const ConstMRef &ml){
  if(dryrun){
    assert(prefix_idx+1 < int(prefix.size()));
    assert(dry_sleepers <= prefix[prefix_idx+1].sleep.size());
    IPid pid = prefix[prefix_idx+1].sleep[dry_sleepers-1];
...
void DelayTraceBuilder::register_alternatives(int alt_count){
  curnode().may_conflict = true;
...
VecSet<DelayTraceBuilder::IPid> DelayTraceBuilder::sleep_set_at(int i){
  VecSet<IPid> sleep;
  for(int j = 0; j < i; ++j){
...
void DelayTraceBuilder::see_events(const VecSet<int> &seen_accesses){
...
void DelayTraceBuilder::add_branch(int i, int j){
  assert(0 <= i);
  assert(i < j);
  assert(j <= prefix_idx);
...
bool DelayTraceBuilder::has_pending_store(IPid pid, void const *ml) const {
...
void DelayTraceBuilder::wakeup(Access::Type type, void const *ml){
  IPid pid = curnode().iid.get_pid();
  ...
  default:
    throw std::logic_error("DelayTraceBuilder::wakeup: Unknown type of memory access.");
  }
...
bool DelayTraceBuilder::has_cycle(IID<IPid> *loc) const{
...
int DelayTraceBuilder::estimate_trace_count() const{
  return estimate_trace_count(0);
}

int DelayTraceBuilder::estimate_trace_count(int idx) const{
